...
describe('parse()', () => {
    ...
    it('errors when content-length header greater than maxBytes (file)', async () => {
        ...
        const err = await expect(Subtext.parse(request, null, { parse: false, output: 'file', maxBytes: 10 })).to.reject();
        expect(err.output.statusCode).to.equal(413);
        expect(err.message).to.include('Payload content length greater than maximum allowed: 10');
    });
